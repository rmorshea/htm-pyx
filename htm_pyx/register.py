...
def html_decode(input: bytes, errors: str = "strict") -> Tuple[str, int]:
    value = transpile(utf_8.decode(input, errors)[0])
    ...
    def decode(self, input: bytes, final: bool = False) -> str:
        try:
            ...
            if final:
                buff = self.buffer
                self.buffer = b""
                result, _ = html_decode(buff, self.errors)
            else:
                result = ""
            return result
        except Exception:
...
class HtmlStreamReader(utf_8.StreamReader):
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        try:
            codecs.StreamReader.__init__(self, *args, **kwargs)
            self.stream: StringIO = StringIO(transpile(self.stream.read()))
        except Exception:
            traceback.print_exc()
            raise
...
try:
    ...
    def register_to_ipython_shell(shell: Optional[InteractiveShell] = None) -> None:
        ...
        def wrapper(
            ...
            try:
                transpiled_cell = transpile(raw_cell)
